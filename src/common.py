...
def urlopen_with_retry(*args, **kwargs):
    ...
    for i in range(retry_time):
        ...
        # try to tackle youku CDN fails
            ...
            print('HTTP Error with code{}'.format(http_error.code))
            ...
            if not force and (file_size == os.path.getsize(filepath) or skip_existing_file_size_check):
                ...
                if not is_part:
                    ...
                    if log.yes_or_no('File with this name already exists. Overwrite?'):
                        ...
                        thread.process_signal.emit('Overwriting %s ...' % tr(os.path.basename(filepath)))
                    else:
                        return
...
def download_urls(
    ...
    if len(urls) == 1:
        ...
        if 'av' in kwargs and kwargs['av']:
            from src.processor.ffmpeg import has_ffmpeg_installed
            if has_ffmpeg_installed():
                from src.processor.ffmpeg import ffmpeg_concat_av
            ...
            try:
                from src.processor.ffmpeg import has_ffmpeg_installed
                if has_ffmpeg_installed():
                    from src.processor.ffmpeg import ffmpeg_concat_flv_to_mp4
                    ffmpeg_concat_flv_to_mp4(parts, output_filepath)
                else:
                    from src.processor.join_flv import concat_flv
            ...
            try:
                from src.processor.ffmpeg import has_ffmpeg_installed
                if has_ffmpeg_installed():
                    from src.processor.ffmpeg import ffmpeg_concat_mp4_to_mp4
                    ffmpeg_concat_mp4_to_mp4(parts, output_filepath)
                else:
                    from src.processor.join_mp4 import concat_mp4
            ...
            try:
                from src.processor.ffmpeg import has_ffmpeg_installed
                if has_ffmpeg_installed():
                    from src.processor.ffmpeg import ffmpeg_concat_ts_to_mkv
                    ffmpeg_concat_ts_to_mkv(parts, output_filepath)
                else:
                    from src.processor.join_ts import concat_ts
...
def download_rtmp_url(
    ...
    if player:
        from src.processor.rtmpdump import play_rtmpdump_stream
        play_rtmpdump_stream(player, url, params)
        return

    from src.processor.rtmpdump import (
...
def download_url_ffmpeg(
    ...
    from src.processor.ffmpeg import has_ffmpeg_installed, ffmpeg_download_stream
