...
from src.common import match1, maybe_print, download_urls, get_filename, parse_host, set_proxy, unset_proxy, get_content, \
    dry_run, player
...
class VideoExtractor():
    ...
    def p(self, stream_id=None):
        ...
        maybe_print("title:               %s" % self.title)
        context['site'] = self.__class__.name
        context['title'] = self.title
        if stream_id:
            # Print the stream
            print("stream:")
            context['stream'] = self.p_stream(stream_id)
